...
import { z } from "zod";
import { calculateWalkingCalories, walkingPaceLabels, type WalkingPace } from "@/lib/formulas";
...
const paces: WalkingPace[] = ["slow", "moderate", "brisk", "very_brisk"];

export function WalkingCaloriesForm() {
  const [pace, setPace] = React.useState<WalkingPace>("moderate");
  const {
    register,
    watch,
    formState: { errors },
  ...
  const calories = valid ? calculateWalkingCalories(values.weightKg, pace, values.durationMinutes) : null;
      ...
      <Card className="p-6">
        <form className="space-y-4">
          ...
          <label className="block">
            <span className="text-sm font-medium text-foreground/80">Walking pace</span>
            <select
              value={pace}
              onChange={(e) => setPace(e.target.value as WalkingPace)}
